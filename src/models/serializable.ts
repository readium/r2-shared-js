...
const _keyToPreserveUnknownJSON = "AdditionalJSON";

export interface IWithAdditionalJSON {
    [_keyToPreserveUnknownJSON]: JsonMap | undefined;
...
export function TaJsonDeserialize<T>(json: any, type: TConstructor<T>): T {
    return TAJSON.deserialize<T>(json, type, { keyToPreserveUnknownJSON: _keyToPreserveUnknownJSON });
}

export function TaJsonSerialize<T>(obj: T): JsonMap {
    return TAJSON.serialize(obj, { keyToPreserveUnknownJSON: _keyToPreserveUnknownJSON }) as JsonMap;
}
